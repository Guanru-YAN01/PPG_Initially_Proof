...
class UNetGenerator1D(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_filters=2, kernel_size=3, stride=2, padding=1):
        ...
        self.dec7 = self._deconv_block(base_filters*256, base_filters*64, kernel_size, stride, padding)
        self.dec6 = self._deconv_block(base_filters*128, base_filters*32, kernel_size, stride, padding)
        self.dec5 = self._deconv_block(base_filters*64, base_filters*16, kernel_size, stride, padding)
        self.dec4 = self._deconv_block(base_filters*32, base_filters*8, kernel_size, stride, padding)
        self.dec3 = self._deconv_block(base_filters*16, base_filters*4, kernel_size, stride, padding)
        self.dec2 = self._deconv_block(base_filters*8, base_filters*2, kernel_size, stride, padding)
        self.dec1 = self._deconv_block(base_filters*4, base_filters, kernel_size, stride, padding)

        # Final output conv
        self.final = nn.Sequential(
            nn.Conv1d(base_filters*2, out_channels, kernel_size=kernel_size, stride=1, padding=padding),
            nn.Tanh()
        )
    ...
    def forward(self, x):
        # Encoder
        e1 = self.enc1(x)  # -> [B,2,L/2]
        e2 = self.enc2(e1) # -> [B,4,L/4]
        e3 = self.enc3(e2)
        e4 = self.enc4(e3)
        e5 = self.enc5(e4)
        e6 = self.enc6(e5)
...
# Example usage:
# model = UNetGenerator1D()
# input_tensor = torch.randn((16, 1, 512))  # batch of 16, 1D signal, length 512
